...
import com.autofactory.controller.rest.EngineController;
...
public class EngineService implements EngineController, Serializable {

    @Inject
    EngineDAO dao;

    @Override
    public Response create(Engine entity) {
        dao.save(entity);
        return Response.ok().build();
    }

    @Override
    public Response update(long id, Engine entity) {
        dao.update(entity);
        return Response.ok().build();
    }
    ...
    public Response getById(long id) {
        Response response;
        Engine engine = dao.getById(id);
        if (engine != null) {
            response = Response.ok().entity(engine).build();
        } else {
